    ...
    <footer className="bg-dark text-dark-muted">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-10 lg:gap-8">
          ...
          <div className="lg:col-span-1">
            <Link href="/" className="flex items-center gap-2 mb-4">
              <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center">
                <svg viewBox="0 0 24 24" fill="none" className="w-5 h-5 text-dark-text">
              ...
              <span className="text-dark-text text-lg font-bold tracking-tight">
            ...
            {hasSocials && (
              <div className="flex items-center gap-3 mt-5">
                {hasInstagram && (
                  <a href={s.instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="w-9 h-9 rounded-lg bg-dark-surface hover:bg-primary/20 flex items-center justify-center transition-colors group">
                ...
                {hasTelegram && (
                  <a href={s.telegram} target="_blank" rel="noopener noreferrer" aria-label="Telegram" className="w-9 h-9 rounded-lg bg-dark-surface hover:bg-primary/20 flex items-center justify-center transition-colors group">
                    <svg className="w-[18px] h-[18px] text-gray-400 group-hover:text-white transition-colors" fill="currentColor" viewBox="0 0 24 24">
                ...
                {hasWhatsapp && (
                  <a href={s.whatsapp.startsWith("http") ? s.whatsapp : `https://wa.me/${s.whatsapp.replace(/[^0-9]/g, "")}`} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" className="w-9 h-9 rounded-lg bg-dark-surface hover:bg-primary/20 flex items-center justify-center transition-colors group">
                    <svg className="w-[18px] h-[18px] text-gray-400 group-hover:text-white transition-colors" fill="currentColor" viewBox="0 0 24 24">
          ...
          {/* Platform */}
          <div>
            <h3 className="text-dark-text text-sm font-semibold uppercase tracking-wider mb-4">
              {t("footer.platform")}
            </h3>
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-sm">
              ...
              <li className="text-dark-muted/60">{s.address}</li>
            </ul>
          </div>
        </div>

        {/* Bottom bar */}
        <div className="mt-12 pt-8 border-t border-dark-border flex flex-col sm:flex-row items-center justify-between gap-3 text-sm">
          <span>&copy; {new Date().getFullYear()} {s.site_name}. {t("footer.rights")}</span>
          <span className="text-dark-muted/60">{t("footer.madeIn")}</span>
        </div>
      </div>
    </footer>
  );
