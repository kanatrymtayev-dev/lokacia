// Run: SUPABASE_SERVICE_KEY=your_key_here node seed.mjs
...
if (!key) {
  console.error("Usage: SUPABASE_SERVICE_KEY=your_key node seed.mjs");
  console.error("Find the key in: Supabase Dashboard → Settings → API → service_role secret");
  process.exit(1);
}
...
if (!hostId) {
  ...
  const existing = users?.users?.find((u) => u.email === "seed-host@lokacia.kz");
  if (!existing) { console.error("Cannot find host user"); process.exit(1); }
  var finalHostId = existing.id;
} else {
  var finalHostId = hostId;
}

console.log("Host ID:", finalHostId);
...
for (const listing of listings) {
  const { error } = await supabase.from("listings").upsert(
    { host_id: finalHostId, status: "active", ...listing },
    { onConflict: "slug" }
  );
  if (error) {
