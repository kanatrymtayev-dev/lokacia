...
let hostUser = userList?.users?.find((u) => u.email === "seed-host@lokacia.kz");

if (!hostUser) {
  const { data, error } = await supabase.auth.admin.createUser({
    email: "seed-host@lokacia.kz",
    password: "seedhost123",
    email_confirm: true,
    user_metadata: { name: "LOKACIA Team", role: "host" },
  });
